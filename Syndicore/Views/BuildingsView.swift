...
struct BuildingsView: View {
                ...
                ForEach(gameData.buildings.fixed.sorted(by: { $0.key < $1.key }), id: \.key) { name, building in
                    NavigationLink {
                        ...
                        HStack {
                            ...
                            Spacer()
                            Text(costSummary(building.baseCost))
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                    }
    ...
    private func costSummary(_ cost: [String: Int]) -> String {
        cost.sorted(by: { $0.key < $1.key })
            .map { "\($0.value) \($0.key)" }
            .joined(separator: ", ")
    }
}

// MARK: - Resource Building Detail
